 ...
 * blocked. The ledger is shared and mutable: move an entry with update()
 * instead of recording a second row for the same surface.
 */
...
export const OUTCOMES = ['clean', 'finding', 'needs_follow_up', 'blocked'] as const
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        ...
        'needs_follow_up | blocked. needs_follow_up marks an open_proof_gap for later agents.',
      parameters: {
        ...
        outcome: { type: 'string', description: 'clean | finding | needs_follow_up | blocked (record).' },
        evidence_note: { type: 'string', description: 'Short note: what was tested, what was observed, or why blocked (record).' },
      },
      output: {
        schema: { type: 'string' },
      ...
      async execute(raw: Record<string, unknown>, exec): Promise<string> {
        ...
        if (args.action === 'list') {
          ...
          return [
            `${entries.length} surface(s) recorded, ${followUps} needing follow-up.`,
            ...lines,
          ].join('\n')
        }
