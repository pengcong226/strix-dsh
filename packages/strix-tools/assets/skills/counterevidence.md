...
reachable. File it with `strix_finding`.
...
`strix_coverage record(outcome="needs_follow_up")`, carry it up in
`完成子任务（汇报结果并结束）(open_items=[...])`, and reflect it in `counterevidence` /
...
gets a `strix_coverage record` entry:
...
they lacked, move their entry with `strix_coverage update` rather than
