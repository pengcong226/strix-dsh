...
There is no CVE and no vulnerable installed version here, so this does not go through `strix_finding (vulnerability_type=dependency_cve)`; that tool requires an advisory-matched CVE. Use `strix_finding` only after the applicable core condition is fully verified.
