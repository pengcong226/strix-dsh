...
cache, and registry configuration. `skill(["npx_confusion"])` when a bare
...
dynamic validation" rule below: report each one with `strix_finding (vulnerability_type=dependency_cve)`
(not `strix_finding`), setting `advisory_cvss` from the published
advisory. `skill(["dependency_cve_scanning"])` for the full SCA workflow.
