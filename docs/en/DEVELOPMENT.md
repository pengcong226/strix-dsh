...
>
> Baseline: dsh CLI **`0.1.5-rc.2`** (upgraded and verified at 0.12.2; since 0.12.8 the peer dual-version ranges also cover **`0.1.6-alpha.1`** — upstream renamed `dsh-code-runtime` to `dsh-ptc-runtime` there, with zero changes to the plugin-facing API/preset/validation rules; only package names and versions needed adapting; since 0.12.10 the peer triple-version ranges also cover **`0.1.6-alpha.2`** — that release deleted the desktop peer-version gate in favor of runtime moduleFallback resolution, with zero changes to the plugin-facing API, verified by a live CLI boot printing the full registration line; since 0.13.0 the peer ranges cover **`0.1.7-alpha.2`**, and since 0.13.1 the five-version ranges cover **`0.1.7-rc.1`** — that release brought peer-compatibility enforcement back (non-satisfying plugins get their rows disabled unless an exact-version exemption is granted) — that release restructured dsh-jobs (JobHandle/ring/SessionId fences) and replaced directory presets with `@deepseek-ai/dsh-agent-preset` declaration rows; the plugin bridges both with a jobs dialect adapter plus a self-disabling preset declaration row, live-verified on both runtimes). dsh is in developer preview — **a heavily changed release every few days is normal**, and §3.9 exists for exactly that.
>
...
|---|---|
| **0.13.2** (09-24) | **Full-review fix batch (P1×1 + P2×1 + P3×4)**: (P1) **`strix_depcheck` silently dropped vuln rows after the enrichment deadline** — the pool's claim closure put `if (expired()) return` BEFORE `rows.push` while the comment promised "degrades to vuln-id-only (still listed)": vulns claimed past the deadline never landed (a repro script showed OSV returning 30 vulns with only 12 listed, the header still claiming "12 known vuln(s)" as complete), and an expired claim then read `rows[rows.length-1]` — another worker's row — re-enriching it (30 detail fetches serving 12 rows); the `[enrichment budget: N row(s)…]` note counts rows inside `rows`, so dropped rows kept it from ever appearing. Fix: the claim pushes the vuln-id-only row unconditionally and **returns that row's reference** (runItem no longer guesses from the array tail), with the `expired()` check moved to the head of the enrich step — post-deadline items degrade honestly and the note counts again. New regression test (mocked fetch: 2 packages × 6 vulns, 80ms details, 50ms budget → asserts all 12 rows listed + the note naming 6 vuln-id-only). (P2) `strix_report finish` now appends the close section via `writeFileAtomic` (the regeneration path was already atomic; the most important write of the engagement was the one that could tear). (P3) `authorization.json` set is atomic too (a torn read already failed closed to passive-only — consistency cleanup); SARIF `coverageKind` maps `blocked` to `open` instead of `notApplicable` (a control-blocked surface is unresolved work like needs_follow_up; ruled_out stays notApplicable) + test case; the headless overlay (`strix-operator.patch.yml`) joins the preset drift fence (previously only patch↔directory were locked, the overlay was hand-aligned and could drift silently — its persona + toolFilter are now compared field-by-field against the directory form); style-noise cleanup (proxy.ts `sidecarState` inline statement / dead `void dir`, http.ts dynamic imports inside execute replaced by static ones, browser.ts dropped the unconsumed `playwrightUnavailable` export). vitest 206 (203 pass + 3 environmental skips). |
| **0.13.1** (09-24) | **dsh 0.1.7-rc.1 adaptation (five-version peer range)**: rc.1 carries **zero plugin-facing API changes** vs alpha.2 (src of core/agent, core/tools, jobs, user-approval, skill untouched; cordis 4.0.4 / schemastery 3.18.4 identical) — the only structural change is the **peer-compatibility enforcement returning**: app-boot gained compatibility-preflight (`evaluatePluginCompatibility` checks every @deepseek-ai/dsh* peer with semver.satisfies(includePrerelease) against the runtime version and disables the plugin row on mismatch, for both profile rows and preset-mount rows; exemptions are exact plugin@version → exact runtime-version grants via `dsh plugin allow-version` or the plugin manager). Adaptation: the 5 dsh-* peers gain `|| 0.1.7-rc.1`; devDeps bumped to rc.1 (tsc clean); vitest 203 unchanged. **All 6 desktop build-patch anchors remain intact for an rc.1 rebuild** (three target files changed in unrelated areas); still no official desktop installer. |
| **0.13.0** (09-23) | **dsh 0.1.7-alpha.2 adaptation (quad peer range + preset-system migration + jobs dual dialect)**: 1461 commits between 0.1.6-alpha.2 and 0.1.7-alpha.2, audited package by package — **the only hard break is the dsh-jobs API restructure** (`JobStart`→`JobSpec`: the starter receives a `JobHandle`, the `readOutput()` hook is gone in favor of a registry-owned ring, and the owner/caller fence switched from Agent objects to SessionId strings); everything else is compatible (cordis 4.0.2→4.0.4 adds only a Volatile type re-export; schemastery 3.18.2→3.18.4; dsh-tools adds only optional `projectContent`/`deferLoading`; dsh-agent/dsh-skill/dsh-user-approval src unchanged; the `readBytes` unification does not apply — the plugin never uses the dsh fs service; no spill-policy keys). **Preset-system breaking migration**: 0.1.7 no longer reads `~/.dsh/.agent-presets` directory presets; presets are now declared by an `@deepseek-ai/dsh-agent-preset` composition row (its `config.plugins` rows use the same format as the old agent.cordis.yml — the same mechanism the official standard/ptc/minimal/cordis presets use). Adaptation: `cordis.patch.yml` gains a `preset-strix` declaration row (its 18 plugin rows parse field-identical to the directory form; the `disabled` guard probes for the declaration package via `createRequire(baseUrl).resolve` — self-disabling on ≤0.1.6 runtimes where the package is absent so the directory preset keeps serving, enabled on 0.1.7+; both directions evaluated live); the `presets/strix/` directory form is retained for ≤0.1.6; a new preset-consistency test fences drift (edit one copy without the other and the suite goes red). Jobs dialect adapter: `'events' in ctx.jobs` feature detection (a 0.1.7-registry-only property), with `startBackgroundShell`/`convergeJobsAtFinish` branching (0.1.7: owner=`agent.id` + `job.append` push into the ring + a one-shot 400KB truncation note; ≤0.1.6: the original readOutput pull semantics preserved verbatim). New features adopted: plugin-manager locale metadata (`locale/en.json` + `zh.json`) and `icon.svg` (the package.json `icon` field, ≤256KiB SVG); `deferLoading` researched and **rejected** (both DeepSeek protocols refuse deferred tool loading requests); volatile config fields deferred (the read shape becomes `.get()` — large churn for little gain). **Live dual verification**: on a 0.1.7-alpha.2 CLI in an isolated home — the roster lists `strix` (no broken flag), `session/create agentPreset=strix` succeeds, and the desktop-isolated layout (plugin not in bundles, declaration row in the profile patch) prints the registration line **exactly once**; on 0.1.6-alpha.2 with the same plugin build — a clean boot (the guarded row produces zero errors/warnings) and the directory preset mounts the full 16 tools + 75 skills on session create. **Deployment notes**: under the bundle-install layout the boot registration line appears twice (host row + eager preset mount) by design; under the desktop-isolated layout (bffa682) the plugin patch never applies, so **a 0.1.7 desktop upgrade MUST add the preset-strix row to the profile `cordis.patch.yml`** (see docs/desktop-upgrade-0.1.7.md). vitest 203 (200 pass + 3 environmental skips). |
| **0.12.11** (09-21) | **Full-review fix batch (P1×1 + P2×2 + P3×4)**: (P1) **the 0.12.9 finish convergence was a silent no-op against the real runtime** — `convergeJobsAtFinish` called `ctx.jobs.list/wait/kill` without a caller, and dsh-jobs-local's `list(caller)` returns only UNOWNED jobs (every strix-shell background job is owned) → it always reported "no live jobs"; the unit tests missed it because the fakes did not implement the owner fence (faithful to the assumption, not the real contract). Fix: `exec.agent` threads through as the caller (all three calls), `jobs.ts` gains a process-wide `trackedShellJobs` bookkeeping map (records id/label/owner at start, drops on settle, shared across all agents in the process), and finish reports live jobs owned by OTHER agents honestly as `NOT convergable` lines naming their owners (the registry forbids cross-agent kills by design); tests rewritten against an owner-fenced fake plus two new cases ("caller's own job converges", "other-owned job reported honestly"). (P2) `strix_proxy` port now publishes as `127.0.0.1:${port}:8080` (a bare `-p` bound all interfaces, contradicting the localhost-only claim; 0.12.7 had fixed only detection, not binding), with `dockerPsLineMatchesPort` accepting the `127.0.0.1:PORT->` form (containers started by older plugin versions still match). (P2) `files` whitelist gains `LICENSE`/`NOTICE` (npm pack proved NOTICE was absent from the tarball; Apache-2.0 §4(d) distribution requirement). (P3) architecture.md version pins aligned with the 0.12.10 triple range; `parseRawRequest` formatting fix + the raw_request description now states the Host-form plaintext-http default; browser screenshot/content drain guardNotes (verdicts no longer display one action late). vitest 191 (188 + 3 environmental skips). |
| **0.12.10** (09-21) | **dsh 0.1.6-alpha.2 adaptation (triple peer range)**: surface-by-surface audit of alpha.1→alpha.2 — the 7 peer packages (cordis/dsh-agent/dsh-jobs/dsh-skill/dsh-tools/dsh-user-approval/schemastery) carry **only version bumps plus one irrelevant cordis logger fix** (an exporter-disposer closure bug); defineTool/ToolRunContext/approval/jobs/skills/systemPrompt/preset row formats unchanged. **Structural change: alpha.2 DELETED the desktop's `validateDesktopPluginGraph` peer-version gate** (726 net lines removed from profile-packages.ts/project-manager.ts), replaced by runtime moduleFallback resolution in `packages/boot/app-boot` — peerDependencies now only declare what the plugin imports (a BFS links host copies), **with no semver gate**; alongside it came the desktop Plugins page (install/configure/live enable-disable) and runtime unload. Adaptation: the 5 dsh-* peers gain `\|\| 0.1.6-alpha.2` (triple range); devDependencies bumped to alpha.2 (tsc clean against the new types); vitest 190 (187 + 3 environmental skips). **Live verification**: `npx @deepseek-ai/dsh@0.1.6-alpha.2 --profile strix web` boots and prints the full registration line `registered 16 tool modules + methodology + authorization sections + 75 skills`. Runtime disable/enable cycle: the plugin holds zero module-level resources (no timers/listeners/resident processes — all registrations are pure), so the unload/reload risk surface is nil; desktop-UI testing waits for a desktop upgrade (alpha.2 has no official installer yet — still a source build). |
| **0.12.9** (09-21) | **Phase 3 landed: structured evidence + finish as a real terminal state**: (1) `strix_finding` gains `evidence_refs` (`[{artifact, source, note?}]` structured artifact references): the artifact must be a file that ALREADY exists inside the workspace (safeWorkspacePath escape rejection + existsSync missing-file rejection, whole-filing fail-closed), and **sha256/registered_at are computed and stamped by the plugin at registration** (a model-claimed hash attests nothing); report generation re-hashes and flags tampered/missing artifacts with honest ⚠ drift lines — a tamper-evident evidence chain. (2) `cvss_vector` syntax validation (new pure `validateCvssVector`: `CVSS:3.1/` prefix + metric-key allowlist + single-letter values + all eight base metrics; malformed vectors rejected on both create and update; the metric-to-evidence mapping remains prompt discipline). (3) `strix_report finish` upgraded from "append four sections" to a real terminal state: live strix-shell jobs are converged before the close (new `convergeJobsAtFinish`: `ctx.jobs.list` → bounded `wait` under the new `finishJobWaitMs` config, default 10s → `kill` the stragglers and record each one in the close section's `### Convergence`; registry unavailability is noted honestly as a skip), remaining loose ends are accounted for (`### Loose Ends`: needs_follow_up/blocked surfaces listed, capped at 10 with an overflow line), a frozen `report-final.md` copy is written, and an existing SARIF sidecar is refreshed so the delivered pair is consistent. (4) `strix_runs` shows `engagement: CLOSED` (report-final.md present = closed, preventing silent continued testing of a finished engagement). **Not done**: a hard post-finish write lock (trust-boundary class, shelved per the standing decision). vitest 190 (187 pass + 3 environmental skips, +13 net: refs normalization/drift/create+report integration, CVSS, finish convergence incl. settle-no-kill/timeout-kill/SARIF refresh/idempotency). |
| **0.12.8** (09-17) | **dsh 0.1.6-alpha.1 adaptation (dual-version transition)**: 800 commits between rc.2 and 0.1.6-alpha.1, surface-by-surface audit conclusion — every API the plugin uses (defineTool/ToolRunContext/ctx.approval.request/ctx.jobs.start/ctx.skills.register/ctx.systemPrompt.section) changed **additively only** (optional `schema` field on ToolExecutionInput, optional `interpolate` on PromptSection; approval/jobs/persona/tool-subagent config and preset row formats unchanged). The only breaking change is the **package rename `@deepseek-ai/dsh-code-runtime` → `@deepseek-ai/dsh-ptc-runtime`** plus the version bump to 0.1.6-alpha.1. Adaptation: peerDependencies slimmed to the 7 actually-imported packages (cordis/dsh-agent/dsh-jobs/dsh-skill/dsh-tools/dsh-user-approval/schemastery — the 8 non-imported transitive type deps moved back to devDependencies, dropping out of the peer surface entirely and dissolving the rename problem); every dsh-* peer now uses the dual range `0.1.5-rc.2 \|\| 0.1.6-alpha.1` (the desktop validates with standard semver.satisfies, so `\|\|` ranges satisfy both hosts — both desktop generations work during the transition); devDependencies bumped to 0.1.6-alpha.1 with the ptc-runtime rename (tsc compiles directly against the new types as compatibility proof). Verification: tsc clean (against 0.1.6 types) + vitest 177 (174 pass + 3 environmental skips, Docker Desktop not running); **simulated validation using the REAL 0.1.6-alpha.1 validateDesktopPluginGraph source (extracted from the tag and compiled): PASS on both the rc.2 and 0.1.6-alpha.1 host inventories, with a negative control (old exact-pin peers against the 0.1.6 host) failing exactly as expected on the peer-range check**; host-driver verification confirms 0.12.8 creates sessions on all three presets on the current rc.2 desktop. Once the local desktop updates to 0.1.6, the plugin needs no further changes. |
| **0.12.7** (09-13) | **Full-review fix batch (leftovers after the upstream 0.1.5-rc.2 fit audit passed)**: (P1) `strix_depcheck` check enrichment (OSV detail + EPSS per vuln) now runs through a 6-lane bounded pool (new pure `runPool`) under one overall `depcheckTimeoutMs` deadline (default 120s, new config field); rows claimed after the deadline degrade honestly to vuln-id-only with an output note — the previous serial unbounded loop was the suite's only tool that could park a call for minutes. (P1) `strix_http` save_to made honest: `sendHttpRequest` now returns a `byteCapped` flag, and when reception was cut at `httpMaxBodyBytes` the saved copy is explicitly annotated as bounded (the description no longer claims "full response body"). (P3) `dockerPsLineMatchesPort` pure matcher accepts both `0.0.0.0:PORT->` and `[::]:PORT->` docker ps publish rows (IPv6-only daemons were missed). (P3) sidecar liveness: a live pid is only trusted after `pidOwnedByDockerCli` verification (pid reuse no longer reports a dead sidecar as running), falling back to docker ps. (P3) the strix preset's plan-mode section rewritten for the pentest context (no more reference to `ask_user_question`, which this preset deliberately does not mount; exploration now names strix_runs/coverage/finding/notes/threat-model). (P3) upstream standard preset's `present` tool row added (declare report.md/findings.sarif as session deliverables); operator children deny `present` (deliverable declaration belongs to the orchestrator); `strix_report finish` output now reminds to present report.md. vitest 177 (176 + 1 platform skip). |
| **0.12.6** (09-13) | **Desktop peerDependencies contract fix**: the desktop startup validation (validateDesktopPluginGraph) requires host-reserved packages (sharedPackages such as @deepseek-ai/cordis) to never appear in a plugin `dependencies` — they must be declared as `peerDependencies` (the host links provide the instance, peer ranges must satisfy). All 15 framework deps moved into `peerDependencies` (ranges unchanged) and mirrored into `devDependencies` for local tsc/vitest resolution; `dependencies` emptied. Also repaired the desktop profile 241 host-package junctions (previously turned into real directories by mistake, tripping "duplicate or aliased host package"). vitest 172 green. |
| **0.12.5** (09-13) | **Session-log compatibility incident fix (root cause of the desktop history load failure surfacing as gateway/internal)**: session-mirror used to append custom event types `strix/coverage` and `strix/note` to the session log. `Session.append()` has no ignorable option, so custom types land WITHOUT the `ignorable: true` marker, and dsh 0.1.5-rc.2 v0-to-v1 format migration refuses unknown non-ignorable events outright — 34 sessions inheriting those events became unloadable. Data fix: a repair script strips all 213 strix rows from the 34 affected zstd logs, renumbers every event seq (packed chunk rows occupy n..n+len-1 with full-range mapping), rewrites sourceEventSeqs (ranges expanded, per-seq remapped, repacked) and surfaceOp.replace startSeq/endSeq, and re-compresses per the dsh container format (header frame + events frame, checksummed); originals backed up under sessions-backup-strix-fix; a full rescan through the host JsonlSessionPersistence chain confirms 149/149 sessions open. Plugin fix: mirrorEvent is now an explicit no-op (module header documents the incident and both contract violations), call sites stay unchanged and tests pin the no-op contract. vitest 172 (171 + 1 platform skip). |
| **0.12.4** (09-13) | **Review-followup fix batch (authorization/trust-boundary items skipped per operator decision)**: CI — the dockerReady probe only checked that a daemon answers; GitHub's windows runners run a Windows-native daemon (no linux/amd64 manifests, python:3.12-slim exits 125), so the two new container integration tests failed on windows legs → probe now requires `docker info --format '{{.OSType}}'` == linux (Docker Desktop Linux containers run as before; Windows-native self-skips). Budget — `budgetAction:'block'` was consulted only by recon/sast/depcheck/proxy-start; the execution tools shell/pybox/browser ran regardless (the config promised more than it enforced) → all three now run checkBudget at entry (block refuses, warn prefixes; browser close is never blocked so cleanup stays available over budget). POST cap — check-then-bump was a soft cap under concurrency (two racers both read seen<cap, both proceeded) → claim-then-check: append the claim first, read back, void any over-cap claim with a `{void:true}` line (the tally subtracts voids, floor zero); at most `cap` valid claims exist per path at any instant. HTTP — response.text() buffered the entire body before the display copy was truncated; a multi-GB response could OOM the host → new `httpMaxBodyBytes` config (default 2MB, 0 = unlimited): streaming reader, cancel at the limit, output stamped `[body reception stopped at N bytes]`. Browser — the `includes('=>')` evaluate heuristic mis-wrapped both bare expressions (IIFE-wrapped into undefined) and statement strings (sent unwrapped, guaranteed SyntaxError) → expressions run directly first, with a one-shot `(() => { ... })()` fallback on page SyntaxError (stamped in the output); concurrent first calls on one session name each launched a browser and the loser leaked (unreachable by close/unload) → single-flight sessionPromises cache. Skills — python.md taught `install_packages: ["requests"]` (an array) while the tool contract is a string → fixed to string examples with the space-separated form spelled out, plus a contract-lint test to catch regressions. vitest 172 (new: hard-cap claim/void offset/byte-cap pair/budget refusals for the three execution tools/evaluate fallback + concurrent single-flight/skill lint). |
| **0.12.3** (09-13) | **Three-track parallel review fix batch (executor/data-plane/engineering subagent deep reads + manual verification)**: P0×3 — `finding` dedupe word-overlap was ALWAYS true for same-endpoint same-type pairs (URL structure words — scheme/host/first segment — are shared by construction, so the second real finding was silently swallowed as a duplicate) → now compares the intersection of DETAIL tokens past the endpoint prefix (query params / deeper segments), bare-vs-detailed treated as ambiguous (file); `coverage` update full-rewrite swallowed concurrent record rows (snapshot-read→rewrite window) → update now appends a version row (`readLedger` resolves by id keeping the last, raw file keeps full edit history); `proxy stop` short-circuited past docker stop after a successful CLI kill (ghost container kept listening and capturing) → extracted injectable-effects `stopSidecarWith`, container stop independent of the CLI kill. P1 — `targetCoveredByAuth` bidirectional substring had no word boundary (`example.com` covered `notexample.com`; recon/sast sent out-of-scope traffic) → host-boundary matching + port semantics (portless scope covers any port; port-pinned scope covers only that port); `evaluatePostPolicy` (shared by http/browser/replay) gains branch (0): under a live attestation, writes to hosts outside targets are refused, and pre-approval is host-bound too; POST cap keys normalized via `normalizePathKey` (trailing/duplicate slashes, matrix params, case variants share one budget); browser spray-guard triple gap (guardNotes closure went blind from the second call / route-install failure was fail-open / `page.route` missed popups & service workers) → notes moved onto the Session and drained per call, install failure now rejects the action, `context.route` + `serviceWorkers:'block'`, navigate restricted to http(s) (blocks `file://` local reads); `shell` image rejects `-`-prefixed/whitespace values in EVERY mode (auto-allow prefix pattern + `--privileged` smuggling) and empty image falls back to the default; addon `.req` switched to absolute-form request lines (HTTPS replay no longer downgraded to cleartext) + replay upgrades old captures via flows.jsonl scheme; `writeFileAtomic` retries rename briefly on EPERM/EBUSY (Windows concurrent-reader case; the EXDEV comment was unreachable and wrong); `pybox` files fully pre-validated before the gate (partial-write residue / non-string crash); `depcheck` rejects null elements; config `httpPostCapPerPath`/`budgetLimitUsd` gain `min(0)` (negative values silently disabled protections — now fail loudly at startup); `finding` update refuses blank title/target; `runs` coverage count uses readLedger semantics (version rows/torn lines no longer counted). Engineering: package README/LICENSE/NOTICE added (publish compliance), CI `timeout-minutes: 20`, CONTRIBUTING test count 36+→154, orphan root lockfile removed, README.zh install profile aligned with en. Note: the `--legacy-peer-deps` mention in the 0.12.2 row is superseded by d58be81 (three peers promoted to direct deps, bare `npm ci` passes). vitest 154 (+15 regression cases across five P0/P1 groups). |
| **0.12.2** (09-12) | **dsh baseline upgrade 0.1.2-alpha.5 → 0.1.5-rc.2 (verified live)**: upstream split monolith packages into granular service packages — the plugin adds seven new direct deps (dsh-system-prompt/dsh-user-approval/dsh-agent/dsh-session/dsh-scope/dsh-llm/dsh-util-values; dsh-tools/dsh-jobs/dsh-skill bumped to 0.1.5-rc.2); defineTool / five inject services / preset.yml three keys / cordis.patch.yml mechanism all compatible with zero code changes; the peer chain needs --legacy-peer-deps to install. Verified: tsc clean, vitest 139 passing, npx @deepseek-ai/dsh@0.1.5-rc.2 web loads the plugin live (16 tools + 75 skills in the boot log). Upstream added an Electron desktop app (apps/desktop) — plugin mechanism unchanged, the suite works there as-is |
| **0.12.1** (09-12) | **Handover-doc P0/P1 fixes (driven by WorkBuddy static review)**: P0-1 persistent browser sessions — a session is now a BrowserContext + long-lived Page (navigation/cookies/localStorage survive across calls; login→fill→click→screenshot works), spray-guard installed once per page, evaluate semantics fixed (IIFE wrapping + undefined rendering); 2 real-Chromium integration tests (state persistence + session isolation + close reset), CI installs Chromium. P0-2 skill contracts — rewrote agent-browser.md/python.md (upstream CLI/Caido SDK teaching diverged completely from the real tools; a Chinese phrase had leaked into Python code), manifest descriptions synced, full scan shows no stale references. adapt_skills.py — self-test exit-code propagation (assertion failures no longer leave CI green), missing-source guard, ASCII-mapping regression test (caught and fixed 8 Chinese-phrase mappings). P1 — http timeout now covers body reception (clearTimeout moved after the body read; slow-drip bodies abort on time with a dedicated error message) + slow-drip regression test. vitest 139 passing (+1 platform skip) |
| **0.12.0** (09-04) | **Four-group review fix batch (all 16 tools deep-read)**: P0 — `recon` httpx fed explicitly via `-l subs.txt` (previously idled while reporting live hosts) + regression test; `proxy stop` kills a pid only after the OS confirms it still belongs to a docker CLI (a forged workspace pid never reaches `process.kill`) + nonce on disk; approval summaries split into full-text match vs truncated display (sha256-stamped, `splitApprovalSummary`, `gate-off` also logged); `dockerRun` and background jobs all use `--cidfile` + `docker rm -f` the daemon-side container on timeout/cancel + completion evidence rows for background jobs. Authorization & execution — shared `targetCoveredByAuth` helper wired into recon/sast (no live attestation, no active scan; nuclei restricted to http(s)); pre-approval bodies match exactly; POST policy extracted as `evaluatePostPolicy` shared with proxy replay (verbs POST/PUT/PATCH/DELETE, check inside the helper as a single choke point, counts shared across verbs); `strix_browser` attaches a per-page route interceptor enforcing the same policy with no human involved (over-cap writes aborted, `browserEnforcePostPolicy` to disable); sast split block tables + normalization (`=` values, `--long` forms, attached `-rl100`) + semgrep workspace confinement + `:ro` mounts; depcheck gains a budget gate; recon domain allowlist. Storage — coverage record append-only + torn-line tolerance + safeId/atomic update; budget append-only `budget-records.jsonl` + audited reset; atomic finding update/report/threat-model save, append-only amend, idempotent finish, close-preserving report, corrupt-file-tolerant list/read paths. Contracts — `code_locations` array schema + `confidence` enum checks in both paths; `pyboxExtraPackages` wired + pip allowlist + `safeId` filenames + random run-dir suffix; SARIF version read from package.json; `clampTimeoutMs` on every timeout entry; dropped-entry counts on authorization set receipts. New config: `sastNucleiImage`/`sastSemgrepImage`/`sastNetwork`/`sastExtraMountRoots`/`proxyImage`/`browserEnforcePostPolicy`. 130+ unit cases, vitest 136 passing (+1 platform skip) |
...
| `strix_coverage` | record (append-only) / update (atomic) / list; outcome ∈ clean/finding/needs_follow_up/blocked/ruled_out; torn-line tolerant | V (C-001) |
...
| `strix_budget` | record (append-only `budget-records.jsonl`) / status / reset (audited); baseline ledger `workspace/budget.json`; `budgetLimitUsd`/`budgetInputPer1k`/`budgetOutputPer1k`/`budgetAction` | V (headless: status empty → record $0.0175 → recon refused `BUDGET EXCEEDED` under a $0.0001 cap → reset zeroed; 7 unit cases) |
...
| `strix_depcheck` | action=check (budget-gated) / kev-refresh / status; packages `[{ecosystem,name,version}]`; OSV primary + KEV cache (vulndb/kev.json 24h TTL) + EPSS ordering | V (headless: status missing → kev-refresh 1694 → lodash@4.17.20 check returned 5 vulns with CVE/EPSS/fixed → dedupe-check NOT A DUPLICATE chain; 4 unit cases) |

### 4.2 Full config table (`src/config.ts`)

`workspaceDir` (empty = anchored at `~/.dsh/strix-workspace`), `httpTimeoutMs` 30s, `httpMaxBodyChars` 20k, `httpPostCapPerPath` 5 (`.jsonl` ledger), `shellImage` python:3.12-slim, `shellAllowedImages[]` (enforced only when unattended with approvalGate off; attended runs show the image in the approval prompt for the operator to decide), `shellNetwork` true, `shellTimeoutMs` 120s, `pyboxImage`, `pyboxExtraPackages[]` (merged with per-call `install_packages`), `pyboxNetwork` true, `pyboxTimeoutMs` 60s, `binariesDir` (empty = look in `~/.dsh/bin` then PATH), `reconTimeoutMs` 300s, `nucleiRateLimit` 50, `sastNucleiImage`/`sastSemgrepImage` (official images, `:latest` defaults), `sastNetwork` true, `sastExtraMountRoots[]` (host roots semgrep may scan outside the workspace), `proxyImage` mitmproxy/mitmproxy:latest, `browserHeadless` true, `browserEnforcePostPolicy` true (automated spray-guard over browser-fired writes), `strictEvidence` true, `approvalGate` `'always'` (HITL approval gate, `'off'` = unattended at your own responsibility, `gate-off` still logged as a decision), `approvalAutoAllow[]` (regex pre-approval, default empty = no loosening, **full-text matching** with truncated+sha256 display, hits logged as `auto-allowed`), `finishJobWaitMs` 10s (bounded wait for live strix-shell jobs during finish convergence before killing stragglers). The authorization attestation is **not in config** — it is per-engagement fact, stored in `workspace/authorization.json` (see the authorization section of tools-reference). Every per-call `timeout_ms` is clamped via `clampTimeoutMs` to (0, 1h].

### 4.3 Known defects & tech debt (honest list)

1. ~~`strix_recon`'s httpx phase not independently verified (same runProcess mechanism, low risk)~~ ✅ **Fixed (0.12.0)**: httpx fed explicitly via `-l subs.txt` (`buildHttpxArgs`, locked by a regression test) — it previously idled while reporting live hosts
