...
`workspaceDir` (empty = anchored at `~/.dsh/strix-workspace`), `httpTimeoutMs` 30s, `httpMaxBodyChars` 20k, `httpPostCapPerPath` 5 (`.jsonl` ledger), `shellImage` python:3.12-slim, `shellAllowedImages[]` (enforced only when unattended with approvalGate off; attended runs show the image in the approval prompt for the operator to decide), `shellNetwork` true, `shellTimeoutMs` 120s, `pyboxImage`, `pyboxExtraPackages[]` (merged with per-call `install_packages`), `pyboxNetwork` true, `pyboxTimeoutMs` 60s, `binariesDir` (empty = look in `~/.dsh/bin` then PATH), `reconTimeoutMs` 300s, `nucleiRateLimit` 50, `sastNucleiImage`/`sastSemgrepImage` (official images, `:latest` defaults), `sastNetwork` true, `sastExtraMountRoots[]` (host roots semgrep may scan outside the workspace), `proxyImage` mitmproxy/mitmproxy:latest, `browserHeadless` true, `browserEnforcePostPolicy` true (automated spray-guard over browser-fired writes), `strictEvidence` true, `approvalGate` `'always'` (HITL approval gate, `'off'` = unattended at your own responsibility, `gate-off` still logged as a decision), `approvalAutoAllow[]` (regex pre-approval, default empty = no loosening, **full-text matching** with truncated+sha256 display, hits logged as `auto-allowed`). The authorization attestation is **not in config** — it is per-engagement fact, stored in `workspace/authorization.json` (see the authorization section of tools-reference). Every per-call `timeout_ms` is clamped via `clampTimeoutMs` to (0, 1h].
