...
| 4 | **强制更新策略烘焙**：`.env.windows` 的 `DSH_DESKTOP_MANDATORY_UPDATE_TEST_ORIGIN` 被打包器强制烘焙进应用 manifest 的 `dshMandatoryUpdatePolicy`；`AUTO_UPDATE_ENV=test` 时策略为 `authentication: "feishu-test"`——自建包没有飞书测试登录，启动即 `authentication-required`，**高斯模糊遮罩盖住整个界面 + 点击被模态拦截（系统声）**，什么都操作不了 | 打开即模糊遮罩、无法交互 | 运行时读 manifest 的 `dshMandatoryUpdatePolicy` 键，**缺省即禁用策略**。修复（无需重构建）：`npx @electron/asar extract resources/app.asar resources/app`（extract 会合并 unpacked 部分），编辑 `resources/app/package.json` 删除 `dshMandatoryUpdatePolicy` 键，把 `app.asar`/`app.asar.unpacked` 改名禁用——Electron 优先加载 `app/` 目录，且后续改插件/清单都是纯文件编辑。**未来重构建后必须重做此步**（或给打包脚本打第 6 个补丁跳过策略烘焙） |
