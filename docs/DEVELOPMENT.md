...
>
> 基线版本：dsh CLI **`0.1.5-rc.2`**（0.12.2 升级实测通过；0.12.8 起 peer 双版本范围同时兼容 **`0.1.6-alpha.1`**——上游该版把 `dsh-code-runtime` 更名为 `dsh-ptc-runtime`，插件面 API/preset/校验规则零变化，仅包名与版本适配；0.12.10 起 peer 三版本范围同时兼容 **`0.1.6-alpha.2`**——该版删除桌面端 peer 版本硬校验改运行时 moduleFallback 解析，插件面 API 零变化，真机 CLI 启动注册行完整；0.13.0 起 peer 范围兼容 **`0.1.7-alpha.2`**；0.13.1 起五版本范围兼容 **`0.1.7-rc.1`**——该版把 peer 兼容性强制校验加了回来（不满足即禁用插件行，精确版本豁免除外）——该版重构 dsh-jobs（JobHandle/ring/SessionId 围栏）并废除目录预设改 `@deepseek-ai/dsh-agent-preset` 声明行，插件以 jobs 双方言适配器 + 带自禁用守卫的预设声明行双轨兼容，两个运行时真机验证通过）。dsh 处于 developer preview，**几天一个大幅改动是常态**，第 3.9 节是为此准备的。
...
| 插件版本 | 内容 |
|---|---|
| **0.13.2**（09-24） | **全量 review 修复批（P1×1 + P2×1 + P3×4）**：(P1) **`strix_depcheck` 超时后静默丢弃漏洞行**——enrichment 池的 claim 闭包把 `if (expired()) return` 放在 `rows.push` 之前，而注释承诺"降级为 vuln-id-only 仍列出"：超时后认领的 vuln 根本不落地（复现脚本实测 OSV 返回 30 个漏洞只列出 12 个，首行还以完整口径宣称 "12 known vuln(s)"），且过期认领后 `rows[rows.length-1]` 取到别的 worker 的行、对同一行重复 enrich（30 次 detail 请求服务 12 行）；`[enrichment budget: N row(s)…]` 降级注记按 rows 内行统计，被丢弃的行不在其中，注记一次都不出现。修复：claim 无条件 push vuln-id-only 行并**返回该行引用**（runItem 不再从数组尾猜），`expired()` 检查移到 enrich 步骤开头——超时条目诚实降级、degrade 注记计数恢复正确。新增回归测试（mock fetch：2 包 × 6 漏洞、detail 80ms、预算 50ms → 断言 12 行全列出 + 注记点名 6 行 vuln-id-only）。(P2) `strix_report finish` 关闭段改 `writeFileAtomic`（report 再生成路径早已原子，唯独最重要的关闭追加用裸 `writeFileSync` 可撕裂）。(P3) `authorization.json` set 改原子写（撕裂时 readAuthorization 返回 null 回落 passive-only，失效方向本就关闭，此为一致性补齐）；SARIF `coverageKind` 把 `blocked` 从 `notApplicable` 改 `open`（被控件拦住的未解决面与 needs_follow_up 同类，ruled_out 保持 notApplicable）+ 用例；headless overlay（`strix-operator.patch.yml`）纳入 preset drift fence（此前只锁 patch↔目录版，overlay 靠手工对齐可静默漂移，现 persona+toolFilter 与目录版逐字段比对）；风格噪音清理（proxy.ts `sidecarState` 行内语句/死 `void dir`、http.ts execute 内动态 import 改静态、browser.ts 移除无消费方的 `playwrightUnavailable` 导出）。vitest 206（203 过 + 3 环境性 skip）。 |
| **0.13.1**（09-24） | **dsh 0.1.7-rc.1 适配（五版本 peer 范围）**：rc.1 相对 alpha.2 插件面**零 API 变化**（core/agent、core/tools、jobs、user-approval、skill 的 src 零 diff；cordis 4.0.4 / schemastery 3.18.4 同版），唯一结构性变化是 **peer 兼容性强制回归**——app-boot 新增 compatibility-preflight（`evaluatePluginCompatibility`：每个 @deepseek-ai/dsh* peer 用 semver.satisfies(includePrerelease) 对照运行时版本，不满足即把插件行 disabled，profile 行与预设挂载行都查；豁免走精确 plugin@version→精确运行时版本的授权，`dsh plugin allow-version` 或插件管理页）。适配：5 个 dsh-* peer 加 `|| 0.1.7-rc.1`，devDeps 全量升 rc.1（tsc 零错），vitest 203 不变。**桌面重建到 rc.1 时 6 处构建补丁锚点全部完好可原样重打**（desktop-build-paths/electron-builder-config/prepare-dsh 三文件有无关区域变动）；官方仍无桌面安装包。 |
| **0.13.0**（09-23） | **dsh 0.1.7-alpha.2 适配（四版本 peer 范围 + 预设体系迁移 + jobs 双方言）**：上游 0.1.6-alpha.2→0.1.7-alpha.2 共 1461 提交，插件面逐包核对——**唯一硬破坏是 dsh-jobs API 重构**（`JobStart`→`JobSpec`：starter 收 `JobHandle`、`readOutput()` 钩子废除改 registry 自有 ring、owner/caller 围栏从 Agent 对象改 SessionId 字符串），其余全兼容（cordis 4.0.2→4.0.4 仅 +Volatile 类型导出；schemastery 3.18.2→3.18.4；dsh-tools 仅增量 `projectContent`/`deferLoading`；dsh-agent/dsh-skill/dsh-user-approval src 零变化；`readBytes` 统一不影响——插件不走 dsh fs 服务；spill-policy 键无）。**预设体系破坏性迁移**：0.1.7 不再读 `~/.dsh/.agent-presets` 目录预设，预设改由组合内 `@deepseek-ai/dsh-agent-preset` 声明行注册（`config.plugins` 行格式与旧 agent.cordis.yml 相同，官方 standard/ptc/minimal/cordis 同款机制）。适配：`cordis.patch.yml` 增 `preset-strix` 声明行（18 行 plugins 与目录版经 YAML 解析逐字段一致；`disabled` 守卫用 `createRequire(baseUrl).resolve` 探测声明包——≤0.1.6 运行时无此包自动禁用、目录预设继续服务，0.1.7+ 启用，双向求值实测）；`presets/strix/` 目录版保留服务 ≤0.1.6；新增预设一致性测试做漂移围栏（改一份不改另一份即红）。jobs 双方言适配器：`'events' in ctx.jobs` 特征检测（0.1.7 registry 独有属性），`startBackgroundShell`/`convergeJobsAtFinish` 双分支（0.1.7：owner=`agent.id` + `job.append` 推送 ring + 400KB 截断注记一次；≤0.1.6：原 readOutput 拉取语义原样保留）。新特性采用：插件管理页 locale 元数据（`locale/en.json`+`zh.json`）+ `icon.svg`（package.json `icon` 字段，≤256KiB SVG）；`deferLoading` 调研后**不采用**（DeepSeek 两种协议均拒绝 deferred tool loading 请求）；volatile 配置字段留待后续（读取形状变 `.get()`，改动面大收益小）。**真机双验证**：0.1.7-alpha.2 CLI 隔离环境——roster 含 `strix`（无 broken）、`session/create agentPreset=strix` 成功、桌面隔离布局（插件不进 bundles、预设行进 profile patch）注册行**恰好一次**；0.1.6-alpha.2 CLI 同插件——boot 干净（守卫行零错误零告警）、目录预设建会话后 16 工具+75 skills 注册行完整。**部署要点**：bundle 安装布局下 boot 注册行两次（宿主行 + 预设 eager 挂载）属预期；桌面隔离布局（bffa682）下插件 patch 不应用，**0.1.7 桌面升级必须把 preset-strix 行加入 profile `cordis.patch.yml`**（见 docs/desktop-upgrade-0.1.7.md）。vitest 203（200 过 + 3 环境性 skip）。 |
